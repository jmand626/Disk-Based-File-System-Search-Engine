...
class Test_DocTableReader : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    FILE* f = fopen("../projdocs/unit_test_indices/enron.idx", "rb");
...
TEST_F(Test_DocTableReader, TestDocTableReaderBasic) {
  ...
  ASSERT_TRUE(success);
  ASSERT_EQ(string("../projdocs/test_tree/enron_email/102."), str);
  success = dtr_->LookupDocID(55, &str);
  ASSERT_TRUE(success);
  ASSERT_EQ(string("../projdocs/test_tree/enron_email/149."), str);
