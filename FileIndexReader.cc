...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  setbuf(file_, NULL);
  // Self explanatory from the man page on set buf, null for second arg means
  // no buffering what so ever

  // STEP 2.
  // Read the entire file header and convert to host format.
  Verify333(fread(&header_, sizeof(IndexFileHeader), 1, file_) == 1);
  // Note that this is 1 because we want to read the entire index file at once
  // This is the size of the indexfileheader because both the big file itself
  // and one of the smaller ones inside of it are called an "index"...
  header_.magic_number = ntohl(header_.magic_number);
  header_.checksum = ntohl(header_.checksum);
  header_.doctable_bytes = ntohl(header_.doctable_bytes);
  header_.index_bytes = ntohl(header_.index_bytes);
  // Since the header stores all of these pieces like this instead of the
  // sizes seperately
  ...
  // Thankfully Verify333 has us covered
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      int64_t byte_read = fread(reinterpret_cast<void *>(buf), 1, 512, file_);
      // This is reinterpret casted because fread expects a void star
      int64_t i;
      left_to_read -= byte_read;

      for (i = 0; i < byte_read; i++) {
        // Since recalculating checksum means calling this on EACH BYTE
        // seperately
